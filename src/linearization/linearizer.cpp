...
LinElem* Linearizer::FindOrAdd(Node *node, PtrToLinElemMap &map)
{
  ...
  std::vector<LinElem*> overwriter(node->NumOutputs(), NULL);
  LinElemSet *others = new LinElemSet[node->NumOutputs()];
  for(auto childConn : node->m_children) {
    ...
    if (outElem) {
      ...
      if (childConn->m_n->Overwrites(node, 0)) {
        if (overwriter[childConn->m_num]) {
          if (overwriter[childConn->m_num] != outElem) {
            cout << "two children of set overwrite output\n";
            throw;
          }
        }
        else {
          overwriter[childConn->m_num] = outElem;
        }
      }
      else if (childConn->m_n->IsDataDependencyOfInput()) {
        others[childConn->m_num].insert(outElem);
      }
    }
  }
  ...
  for (int i = 0; i < node->NumOutputs(); ++i) {
	...
	others[i].erase(iter);
      ...
      overwriter[i]->m_preds.insert(overwriter[i]->m_preds.begin(),
				    others[i].begin(),
				    others[i].end());
      for(auto other : others[i]) {
	if (other->m_succ) {
	  cout << "already has succesor\n";
	  throw;
	}
	else
	  other->m_succ = overwriter[i];
      }
    }
  }
  delete [] others;
