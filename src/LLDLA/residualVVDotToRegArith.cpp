...
ResidualVVDotToRegArith::ResidualVVDotToRegArith(Layer fromLayer, Layer toLayer) {
  m_fromLayer = fromLayer;
  m_toLayer = toLayer;
}

string ResidualVVDotToRegArith::GetType() const {
  return "ResidualVVDotToRegArith";
}

bool ResidualVVDotToRegArith::IsResidualVVDot(const VVDot* vvdot) const {
...
bool ResidualVVDotToRegArith::CanApply(const Node* node) const {
  if (node->GetNodeClass() == VVDot::GetClass()) {
    const VVDot* vvdot = static_cast<const VVDot*>(node);
    return IsResidualVVDot(vvdot);
  }
  LOG_FAIL("Bad class for node in ResidualVVDotToRegArith::CanApply");
  throw;
}

void ResidualVVDotToRegArith::Apply(Node* node) const {
