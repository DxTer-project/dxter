		 ...
		 obj_t *C )
{
  ...
  th_shift_start_end_non_linear(&idx1, &dimLen1, TRUE,
				ProcComm, bli_blksz_for_obj( C, gemm_nr));
  /*
#pragma omp critical
  {
    printf("%u -> %u: %u - %u\n", th_global_thread_id(),
	   th_group_id(ProcComm), idx1, dimLen1);
  }
  */
  for ( ; idx1 < dimLen1; idx1 += bs1 ) {
	...
	bli_packm_init_pack( FALSE, BLIS_NO_INVERT_DIAG, BLIS_PACKED_COL_PANELS, 
			     ...
			     gemm_kr, gemm_nr, 
			     &AT_1_1, &packed_B_pan_local_alloc );
	...
	if (th_am_root(ProcComm)) {
	  ...
	  bli_packm_init_pack( FALSE, BLIS_NO_INVERT_DIAG, BLIS_PACKED_ROW_PANELS, 
			       ...
			       gemm_mr, gemm_kr, 
			       &A_B_1_1, &packed_A_blk_local_alloc );
	  th_broadcast_without_second_barrier(ProcComm, 0,
	...
	bli_packm_blk_var2_par( &BLIS_ONE, &A_B_1_1, &packed_A_blk, ProcComm );
	bli_herk_l_ker_var2_par( &BLIS_ONE, &packed_A_blk, &packed_B_pan_L, 
				 &BLIS_ONE, &C_1_B_1_L, (herk_t*)NULL , L2Comm);
  ...
  for ( ; idx1 < dimLen1; idx1 += bs1 ) {
	...
	if (th_am_root(ProcComm)) {
	  ...
	  bli_packm_init_pack( FALSE, BLIS_NO_INVERT_DIAG, BLIS_PACKED_ROW_PANELS, 
			       ...
			       BLIS_BUFFER_FOR_A_BLOCK,
			       gemm_mr, gemm_kr, 
			       &A_T_1_1, &packed_A_blk_local_alloc );
	  th_broadcast_without_second_barrier(ProcComm, 0,
					      (void*)(&packed_A_blk_local_alloc),
					      (void*)(&packed_A_blk), sizeof(packed_A_blk));
	}
	...
	bli_packm_blk_var2_par( &BLIS_ONE, &A_T_1_1, &packed_A_blk, ProcComm );
	bli_herk_u_ker_var2_par( &BLIS_ONE, &packed_A_blk, &packed_B_pan_R, 
				 &BLIS_ONE, &C_1_T_1_R, (herk_t*)NULL , L2Comm);
		 ...
		 obj_t *C )
{
  ...
  for ( ; idx1 < dimLen1; idx1 += bs1 ) {
    ...
    bli_obj_set_struc( BLIS_GENERAL, C_1_B );
    bli_obj_set_uplo( BLIS_DENSE, C_1_B );
    dimLen2 = bli_obj_width_after_trans( AT_B );
    for ( idx2 = 0; idx2 < dimLen2; idx2 += bs2 ) {
      bs2 = bli_determine_blocksize_f( idx2, dimLen2, &AT_B, gemm_kc );
      dim_t idx3, dimLen3, bs3;
      //****
      obj_t AT_B_1;
	...
	//****
	obj_t AT_B_1_1;
	bli_acquire_mpart_b2t( BLIS_SUBPART1, idx3, bs3, &AT_B_1, &AT_B_1_1 );
	obj_t C_1_B_0;
	bli_acquire_mpart_b2t( BLIS_SUBPART0, idx3, bs3, &C_1_B, &C_1_B_0 );
	obj_t C_1_B_1;
	bli_acquire_mpart_b2t( BLIS_SUBPART1, idx3, bs3, &C_1_B, &C_1_B_1 );
	...
	if (th_am_root(ProcComm)) {
	  ...
	  bli_packm_init_pack( FALSE, BLIS_NO_INVERT_DIAG, BLIS_PACKED_ROW_PANELS, 
			       ...
			       BLIS_BUFFER_FOR_A_BLOCK,
			       gemm_mr, gemm_kr, 
			       &AT_B_1_1, &packed_A_blk_local_alloc );
	  th_broadcast_without_second_barrier(ProcComm, 0,
					      (void*)(&packed_A_blk_local_alloc),
					      (void*)(&packed_A_blk), sizeof(packed_A_blk));
	}
	...
	bli_acquire_mpart_l2r( BLIS_SUBPART1,
			       offL, nL, &packed_B_pan, &packed_B_pan_L );
	bli_packm_blk_var2_par( &BLIS_ONE, &AT_B_1_1, &packed_A_blk, ProcComm );
	bli_herk_l_ker_var2_par( &BLIS_ONE, &packed_A_blk, &packed_B_pan_L, 
				 &BLIS_ONE, &C_1_B_1_L, (herk_t*)NULL , L2Comm);
		 ...
		 obj_t *C )
{
  ...
  for ( ; idx1 < dimLen1; idx1 += bs1 ) {
    ...
    bli_obj_set_struc( BLIS_GENERAL, C_1_T );
    bli_obj_set_uplo( BLIS_DENSE, C_1_T );
    dimLen2 = bli_obj_width_after_trans( AT_T );
    for ( idx2 = 0; idx2 < dimLen2; idx2 += bs2 ) {
      bs2 = bli_determine_blocksize_f( idx2, dimLen2, &AT_T, gemm_kc );
      dim_t idx3, dimLen3, bs3;
      //****
      obj_t AT_T_1;
	...
	//****
	obj_t AT_T_1_1;
	bli_acquire_mpart_t2b( BLIS_SUBPART1, idx3, bs3, &AT_T_1, &AT_T_1_1 );
	obj_t C_1_T_1;
	bli_acquire_mpart_t2b( BLIS_SUBPART1, idx3, bs3, &C_1_T, &C_1_T_1 );
	obj_t C_1_T_2;
	bli_acquire_mpart_t2b( BLIS_SUBPART2, idx3, bs3, &C_1_T, &C_1_T_2 );
	...
	if (th_am_root(ProcComm)) {
	  ...
	  bli_packm_init_pack( FALSE, BLIS_NO_INVERT_DIAG, BLIS_PACKED_ROW_PANELS, 
			       ...
			       BLIS_BUFFER_FOR_A_BLOCK,
			       gemm_mr, gemm_kr, 
			       &AT_T_1_1, &packed_A_blk_local_alloc );
	  th_broadcast_without_second_barrier(ProcComm, 0,
					      (void*)(&packed_A_blk_local_alloc),
					      (void*)(&packed_A_blk), sizeof(packed_A_blk));
	}
	...
	bli_acquire_mpart_l2r( BLIS_SUBPART1,
			       offR, nR, &packed_B_pan, &packed_B_pan_R );
	bli_packm_blk_var2_par( &BLIS_ONE, &AT_T_1_1, &packed_A_blk, ProcComm );
	bli_herk_u_ker_var2_par( &BLIS_ONE, &packed_A_blk, &packed_B_pan_R, 
				 &BLIS_ONE, &C_1_T_1_R, (herk_t*)NULL , L2Comm);
...
int main( int argc, char** argv )
{
  obj_t a, ah, c1, c2;
	...
	for ( p = p_begin; p <= p_end; p += p_inc )
	{
		...
		gflopsDxT = ( 1.0 * m * k * m ) / ( dtime_saveDxT * 1.0e9 );
		gflopsBLI = ( 1.0 * m * k * m ) / ( dtime_saveBLI * 1.0e9 );

		printf( "data_herk_DxT" );
		...
		printf( "( %2ld, 1:4 ) = [ %4lu %4lu  %10.3e  %6.3f ];\n",
		        (p - p_begin + 1)/p_inc + 1, m, k, dtime_saveBLI, gflopsBLI );
